...
cell_num = 96309  #中国区总的格网单元数
minimum = 1e-20  #一个极小值

def cal_norm_rainfall_data(path,store_path):
    ...
    for filename in os.listdir(path):
        ...
        index = int(os.path.splitext(filename)[0].split('.')[1])  # 获取格网单元的索引号
        column = int(os.path.splitext(filename)[0].split('_')[2])  # 获取格网单元的列号

        df_test = os.path.getsize(filepath)
        if df_test == 0:
            for i in range(24):
                fre_all[index - 1].append(0)
                amu_all[index - 1].append(0)
                int_all[index - 1].append(0)
        else:
            ...
            for i in range(len(df)):
                ...
                delta_time = round((69.95 + 0.1 * column) / 15)  # 计算该格网单元与北京时间的时差
            ...
            for i in range(24):
                fre_all[index - 1].append(fr_norm_list[i])
                amu_all[index - 1].append(am_norm_list[i])
                int_all[index - 1].append(int_norm_list[i])
    ...
    # 按照不同历时命名文件
    str_path_fre = store_path + "\\" + "08-16" + "_6-8month_yearly_mean_frequency" + ".csv"
    str_path_amu = store_path + "\\" + "08-16" + "_6-8month_yearly_mean_amount" + ".csv"
    str_path_int = store_path + "\\" + "08-16" + "_6-8month_yearly_mean_intensity" + ".csv"
...
if __name__ == '__main__':
    ...
    # path = r"H:\preci-extract\rainfall_variation\data_input_output\test"
    store_path = r"H:\preci-extract\rainfall_variation\data_input_output\kmeans_1211\kmeans_input_data"
    cal_norm_rainfall_data(path, store_path)